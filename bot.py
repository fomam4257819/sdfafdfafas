...
def get_trainer_description(message):
    ...
    try:
        ...
        try:
            # SQL запрос БЕЗ параметров - все значения вставлены с экранированием
            ...
            logger.info(f"📤 Выполнение SQL: {query}")
            ...
            logger.error(f"❌ Помилка БД при INSERT: {db_error}")
...
def send_request_to_trainer(call):
    ...
    try:
        ...
        result = db.execute(
            f"SELECT username, name FROM trainers WHERE id = {trainer_id}"
        )
        trainer = result.rows[0] if result.rows else None
...
Тренер, зв'яжись з учнем!"""
        ...
        try:
            ...
            bot.send_message(
                call.message.chat.id,
                "⚠️ Не вдалось надіслати з��явку тренеру. Перевір контакти адміністратора."
            )
