...
TURSO_TOKEN = os.getenv("TURSO_TOKEN", "eyJhbGciOiJFZERTQSIsInR5cCI6IkpXVCJ9.eyJhIjoicnciLCJleHAiOjE4NzM2NDEyMDEsImlhdCI6MTc3NzIxODgwMSwiaWQiOiIwMTlkY2E3ZC1kNzAxLTcxNjktOGJiYS03MDNkNDVkYmIxYTMiLCJyaWQiOiIwNjRkODk3YS00ZDM3LTQ5MDEtODg5YS1iYjVkNmI5OWZjZTkifQ.Ecd93rj_9Usy0I7-ROEkQyS6AgXoWNXVixsEBhg0sfBEBntPVIPiVoecS5SXRDkZzfV3s8X9teIanxGoKZ6IBw")
MAX_DB_RETRIES = 3
DB_RETRY_DELAY = 2
...
def add_trainer_start(message):
    ...
    # Використовуємо спеціальну кнопку Telegram для гарантованого отримання user_id
    markup.add(
        types.KeyboardButton(
            "👤 Обрати тренера зі списку чатів", 
    ...
    bot.send_message(
        ...
        "┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄\n"
        "🔵 Крок 1 з 3  ●○○\n\n"
        "Натисніть кнопку *«👤 Обрати тренера»* внизу та виберіть потрібну людину зі списку ваших чатів.\n\n"
        "_(Або ви можете просто переслати мені сюди будь-яке повідомлення від цього тренера)_",
        parse_mode="Markdown",
        reply_markup=markup
    )
...
def add_trainer_id_received(message):
    ...
    # 1. Якщо адмін натиснув нову кнопку request_user
    if message.content_type == "user_shared":
        tg_id = message.user_shared.user_id
        # Спробуємо підтягнути дані (працює, якщо тренер вже писав боту)
        try:
            chat_info = bot.get_chat(tg_id)
    ...
    # 2. Якщо адмін переслав повідомлення від тренера
    elif message.forward_from:
        tg_id = message.forward_from.id
        uname = message.forward_from.username or ""
        first = message.forward_from.first_name or ""
        last = message.forward_from.last_name or ""

    # 3. Якщо адмін по-старому відправив контакт (fallback)
    elif message.content_type == "contact" and message.contact.user_id:
        tg_id = message.contact.user_id
        first = message.contact.first_name or ""
        last = message.contact.last_name or ""

    # 4. Якщо адмін просто написав Telegram ID цифрами
    ...
    if not tg_id:
        bot.send_message(
            cid,
            "⚠️ Не вдалося отримати Telegram ID користувача.\n\n"
            "Будь ласка, скористайтеся кнопкою *«👤 Обрати тренера»* або *перешліть мені повідомлення* від нього.",
            parse_mode="Markdown"
        )
        return

    fullname = f"{first} {last}".strip() or "Тренер"
    ...
    bot.send_message(
        ...
        f"_(або надішліть крапку_ `.` _щоб залишити це:_ *{fullname}*_)_",
        parse_mode="Markdown",
...
def user_picked_trainer(call):
    ...
    try:
        ...
        trainer_tg_id    = _unpack_turso_value(result[0][2])
    except Exception as e:
    ...
    if trainer_tg_id:
        try:
            ...
            pass
    if not trainer_notified and trainer_username and trainer_username != "None":
        try:
...
def admin_confirm_enroll(call):
    ...
    if not trainer_notified and trainer_username and trainer_username != "None":
        try:
            bot.send_message(f"@{trainer_username}", confirm_trainer_msg, parse_mode="MarkdownV2")
        except Exception:
            pass
...
def admin_write_reject_reason(message):
    ...
    if not data:
        bot.send_message(message.chat.id, "❌ Помилка: дані не знайдені.", reply_markup=admin_menu_markup())
    ...
    bot.send_message(message.chat.id, "✅ Учня повідомлено про відмову.", reply_markup=admin_menu_markup())
